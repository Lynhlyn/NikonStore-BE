...
public interface CustomerRepository extends JpaRepository<Customer, Integer> {
  ...
  @Query(
      "SELECT c FROM Customer c WHERE "
          ...
          + "AND ((:email IS NULL AND :phoneNumber IS NULL AND :fullName IS NULL) OR "
          + "(:email IS NOT NULL AND :phoneNumber IS NULL AND :fullName IS NULL AND LOWER(c.email) LIKE LOWER(CONCAT('%', :email, '%'))) OR "
          + "(:email IS NULL AND :phoneNumber IS NOT NULL AND :fullName IS NULL AND c.phoneNumber LIKE CONCAT('%', :phoneNumber, '%')) OR "
          + "(:email IS NULL AND :phoneNumber IS NULL AND :fullName IS NOT NULL AND LOWER(c.fullName) LIKE LOWER(CONCAT('%', :fullName, '%'))) OR "
          + "(:email IS NOT NULL OR :phoneNumber IS NOT NULL OR :fullName IS NOT NULL) AND ("
          + "(:email IS NOT NULL AND LOWER(c.email) LIKE LOWER(CONCAT('%', :email, '%'))) OR "
          + "(:phoneNumber IS NOT NULL AND c.phoneNumber LIKE CONCAT('%', :phoneNumber, '%')) OR "
          + "(:fullName IS NOT NULL AND LOWER(c.fullName) LIKE LOWER(CONCAT('%', :fullName, '%'))))) "
